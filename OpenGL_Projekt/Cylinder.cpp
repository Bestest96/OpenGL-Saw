...
Cylinder::Cylinder(): Cylinder(30, 1.0, 5.0)
{}


Cylinder::Cylinder(GLuint pSlices, GLfloat pRadius, GLfloat pLength, GLuint pTextureRepeat): slices(pSlices), radius(pRadius), length(pLength), vertexParams(8), textureRepeat(pTextureRepeat)
{
	const int vertexSize = vertexParams * (2 * (slices + 1));
	...
	{
		const float theta = glm::radians(360.0f * i / slices);
	...
	indices = new GLuint[12 * slices];
	for (auto i = 0; i < slices; ++i)
	{
		indices[12 * i] = 0;
		indices[12 * i + 1] = i + 1;
		indices[12 * i + 2] = (i + 2) > slices ? 1 : i + 2;
		indices[12 * i + 3] = slices + 1;
		indices[12 * i + 4] = slices + 1 + i + 1;
		indices[12 * i + 5] = (slices + 1 + i + 2) > (2 * slices + 1) ? slices + 2 : slices + 1 + i + 2;
		indices[12 * i + 6] = i + 1;
		indices[12 * i + 7] = (i + 2) > slices ? 1 : i + 2;
		indices[12 * i + 8] = slices + 1 + i + 1;
		indices[12 * i + 9] = (i + 2) > slices ? 1 : i + 2;
		indices[12 * i + 10] = slices + 1 + i + 1;
		indices[12 * i + 11] = (slices + 1 + i + 2) > (2 * slices + 1) ? slices + 2 : slices + 1 + i + 2;
	}
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 12 * slices * sizeof(GLuint), indices, GL_STATIC_DRAW);
...
{
	glBindVertexArray(VAO);
	glDrawElements(GL_TRIANGLES, 12 * slices, GL_UNSIGNED_INT, 0);
	glBindVertexArray(0);
}

void Cylinder::calculateVertexNormals() const
{
	const int vertexNormalsCount = 2 * slices + 2;
	glm::vec3 *vertexNormals = new glm::vec3[vertexNormalsCount];
	const int faceNormalsCount = 4 * slices;
	for (int i = 0; i < faceNormalsCount; ++i)
	{
		...
		glm::vec3 insidePoint = { 0.0, 0.0, length / 2 };
